...
import { TokenStatus, ProcurementState, PaymentStatus, BookingStatus, ComplaintStatus } from '@prisma/client';
...
@Injectable()
export class KisanSahayakAgent {
  private readonly logger = new Logger(KisanSahayakAgent.name);

  constructor(private prisma: PrismaService) {}

  async answer(input: SupportInput): Promise<SupportResponse> {
  ...
  private async getTokenStatus(farmerId: string, context: any): Promise<SupportResponse> {
  ...
  private async getAppointmentStatus(farmerId: string, context: any): Promise<SupportResponse> {
  ...
  private async getCenterLocation(farmerId: string, context: any): Promise<SupportResponse> {
  ...
  private async getQueueStatus(farmerId: string, context: any): Promise<SupportResponse> {
    const tokens = await this.prisma.token.findMany({
      where: {
        farmerId,
  ...
  private async getProcurementStatus(farmerId: string, context: any): Promise<SupportResponse> {
  ...
  private async getPaymentStatus(farmerId: string, context: any): Promise<SupportResponse> {
  ...
  private getBookingHelp(farmerId: string, context: any): SupportResponse {
    return {
  ...
  private async getScheduleInfo(farmerId: string, context: any): Promise<SupportResponse> {
  ...
  private helpComplaintCreation(farmerId: string, context: any): SupportResponse {
    return {
  ...
  private getDefaultResponse(question: string): SupportResponse {
    return {
